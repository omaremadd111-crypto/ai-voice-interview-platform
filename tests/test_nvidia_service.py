"""NVIDIA NIM provider contract tests with a deterministic OpenAI-SDK-shaped fake.

NVIDIA NIM is OpenAI-compatible but Nemotron does not natively guarantee
...
of a provider-native parse() call -- the same mechanism
services/llm/openrouter_service.py uses. These tests mirror
tests/test_openrouter_service.py's coverage shape.
"""
import json
...
class FakeNvidiaAPIError(Exception):
    status_code = 429
...
def _service(chat_completions: FakeChatCompletionsAPI) -> NvidiaLLMService:
    return NvidiaLLMService(
        api_key="unit-test-key",
        client=FakeNvidiaClient(chat_completions),
        api_error_types=(FakeNvidiaAPIError,),
    )
...
def test_every_supported_task_produces_valid_structured_output(
    ...
    assert call["model"] == DEFAULT_NVIDIA_MODEL
    assert call["max_tokens"] == 321
    assert call["temperature"] == 0.0
    assert call["response_format"] == {"type": "json_object"}
    # Thinking disabled on every request -- without this, Nemotron spends part
    # of its completion budget on a hidden reasoning trace before ever writing
    # the answer (verified against the live endpoint).
...
def test_configured_model_is_used_when_provided() -> None:
    ...
    service = NvidiaLLMService(
        api_key="unit-test-key",
        model="nvidia/some-other-model",
        client=FakeNvidiaClient(chat),
        api_error_types=(FakeNvidiaAPIError,),
    )

    service.generate_structured(_request(), SampleOutput)

    assert chat.create_calls[0]["model"] == "nvidia/some-other-model"
...
def test_api_errors_are_wrapped_without_exposing_provider_details() -> None:
    chat = FakeChatCompletionsAPI([
        FakeNvidiaAPIError("request failed with unit-test-key and raw candidate text")
    ])
    service = _service(chat)
    ...
    assert message == "NVIDIA API request failed (HTTP 429)."
...
def test_generate_text_uses_same_model_and_never_sends_api_key_in_payload() -> None:
    chat = FakeChatCompletionsAPI(text_outcome="NVIDIA text response")
    service = _service(chat)

    result = service.generate_text(_request())

    assert result == "NVIDIA text response"
    assert chat.create_calls[0]["model"] == DEFAULT_NVIDIA_MODEL
    assert "response_format" not in chat.create_calls[0]
    assert chat.create_calls[0]["extra_body"] == {"chat_template_kwargs": {"enable_thinking": False}}
...
def test_generate_text_error_is_wrapped() -> None:
    chat = FakeChatCompletionsAPI(text_outcome=FakeNvidiaAPIError("boom"))
    service = _service(chat)
...
def test_provider_module_never_logs_anything() -> None:
    ...
    calls (matches services/llm/mistral_service.py and
    services/llm/openrouter_service.py's identical discipline), so there is
    nothing for the logging layer's redaction to ever need to catch here in
    the first place."""
    import inspect

    import services.llm.nvidia_service as module
...
def test_service_rejects_blank_api_key() -> None:
    with pytest.raises(LLMConfigurationError, match="must not be blank"):
        NvidiaLLMService(
            api_key="   ", client=FakeNvidiaClient(FakeChatCompletionsAPI()),
        )


def test_service_defaults_to_the_verified_live_model() -> None:
    service = NvidiaLLMService(
        api_key="unit-test-key", client=FakeNvidiaClient(FakeChatCompletionsAPI()),
    )
...
def test_missing_sdk_raises_configuration_error(monkeypatch: pytest.MonkeyPatch) -> None:
    """Mirrors Mistral/OpenRouter's behaviour: a real-provider service without
    an injected client tries to import the SDK lazily, and a missing SDK must
    ...
    service = NvidiaLLMService(api_key="unit-test-key")
...
# them directly to an NvidiaLLMService backed by a fake client, proving the
# new provider is a drop-in for both without any change to either class.


def test_voice_intent_classifier_works_through_nvidia() -> None:
    chat = FakeChatCompletionsAPI([{
        "intent": "dont_know",
        "confidence": 0.95,
        "reaction": "No problem, let's move on.",
...
def test_interviewer_follow_up_generation_works_through_nvidia() -> None:
